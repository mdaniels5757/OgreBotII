...
((window) => {
    ...
    next.click($.proxy(updatePhase, previous, 1)).
    ...
    $(".download-button").click(function () {
        ...
        $("#recommended-dimensions").html(`${row.data("mapRecommendedWidth")}`
                + `&nbsp;×&nbsp;${row.data("mapRecommendedHeight")}`);
    });
    ...
    $("#load-state").click(function () {
        ...
        $.each(state, (name, entry) => {
            var loadButton =
                $("<input type='button' value='Load'  class='btn btn-primary' />").click(() => {
                    typeCheckbox.filter(`[value='${entry.type}']`).prop("checked", true);
            ...
            var removeButton =
                $("<input type='button' value='Remove' class='btn btn-warning' />").click(() => {
    ...
    saveState.click(() => {
        ...
        while (state[prefix + (++defaultName)]) {
            /* empty block for Eclipse debugger */
        }

        do {
            name = prompt("Name of the saved state: ", `${prefix}${defaultName}`);
            ...
            try {
                ...
                $.each(getSavedStateFromText(text), (key, newState) => {
                    ...
                    if ($.isPlainObject(inputs)) {
                        ...
                        if (validInputs) {
                            existing = state[key];
                            if (!existing || stringify(existing) !== stringify(newState) ||
    ...
    $("#export").click(() => {
        saveAs(new Blob([localStorage.getItem(saveStateStorageName)], {
