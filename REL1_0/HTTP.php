...
class HTTP {
	...
	/* set timeout: Magog 3/20/11 */
	function setTimeout( $length_in_seconds ) {
		curl_setopt($this->curl_instance,CURLOPT_TIMEOUT, $length_in_seconds);
	}
	...
	function get( $url, $data = null ) {
		...
		if (!Environment::prop("environment", "ssl.verify")) {
			curl_setopt($this->curl_instance, CURLOPT_SSL_VERIFYPEER, 0);
		}		
