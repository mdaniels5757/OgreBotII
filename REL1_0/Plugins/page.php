...
class Page {
	 ...
	 * @var string
	 */
	private $default_starttimestamp;
	...
	function __construct( &$wikiClass, $title = null, $pageid = null, $followRedir = true, $normalize = true ) {
		...
		//kill Mediawiki warning message
		$this->default_starttimestamp = $this->wiki->supports_version("1.26.10") ? "now" : "";
	...
	public function edit( 
		...
		elseif( $tokens['edit'] == '' ) {
			if ($logger) {
				$logger->error( "User is not allowed to edit {$this->title}.");
			}
			return false;
		}
		...
		if( mb_strlen( $summary, '8bit' ) > 255 ) {
			if ($logger) {
				$logger->error( "Summary is over 255 bytes, the maximum allowed.");
			}
			return false;
		}
		
		if ($logger) {
			$logger->trace( "Making edit to {$this->title}...");
		}
		...
		$editarray = array(
			...
			'action' => 'edit',
			'token' => $tokens['edit'],
			'starttimestamp' => $this->default_starttimestamp,
			'basetimestamp' => $this->lastedit,
			// commented out md5... causing problems with invalid 
		    // encoding in metadata for certain images
			//'md5' => md5($text),
			'text' => $text,
			'assert' => 'user',
		);
		if (!$editarray['basetimestamp'] && $this->wiki->supports_version("1.26.10")) {
			$editarray['basetimestamp'] = "now";
		}
	...
	protected function get_metadata( $pageInfoArray2 = null ) {
		...
		foreach( $pageInfoRes['query']['pages'] as $key => $info ) {
			...
			if( $this->pageid > 0 ) {
				...
				$this->starttimestamp = (isset($info['starttimestamp'])) ? 
					$info['starttimestamp'] : $this->default_starttimestamp;
			}
			else {
				$this->pageid = 0;
				$this->lastedit = '';
				$this->hits = '';
				$this->length = '';
				$this->starttimestamp = $this->default_starttimestamp;
			}
			...
			//Magog 2014-01-01
			if (isset($logger) && ($this->title === null || $this->namespace_id===null)) {
				$logger->error("title or namespace_id not set. \$key, \$info:");
				$logger->error($key);
				$logger->error($info);
			}
	...
	protected function preEditChecks(){
		...
		if( isset($messages) && $messages && $this->wiki->get_stoponnewmessages() ) {
