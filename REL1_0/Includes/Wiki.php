...
class Wiki {
	...
	const CSRF_TOKEN_TYPES = 'edit|delete|protect|move|block|unblock|email|import';
	...
	/**
	 * A hash string by which to uniquely distinguish this wiki from other 
	 *  instances of wiki created by this thread. This value will always
	 *  be equal to the timestamp at which the class was instantiated.
	 *
	 * @var string
	 * @access protected
	 */
	...
	/**
	 * A hook to be run on this instance of Wiki.
	 * @var callable[][]
	 * @access protected
	 */
	protected $hooks = [];
	 ...
	 * @param array $token Token if the wiki needs a token. Used internally, don't use (default: null)
	 * @param string|null $version 
	 * @return void
	 */
	function __construct($configuration, $extensions = array(), $recursed = 0, array $token = null,
		 $mwversion = null) {
		global $environment, $pgProxy, $pgVerbose;
		...
		if( isset( $configuration['cookiejar'] ) ) {
			...
			$this->http->setCookieJar( TMP_DIRECTORY_SLASH . 'PeachyCookieSite' . sha1( $configuration['encodedparams'] ) );
			
			$use_cookie_login = !$token && boolean_or_exception($environment["wiki.login.use_cookie"]) &&
					$this->is_logged_in();
		}
		...
		$lgarray = array(
			...
			/* Magog 10/18/2011 make sure key exists before setting */
		...
		if( !is_null( $token ) ) {
			$lgarray = array_merge($lgarray, $token);
		}
		...
		if( $use_cookie_login ) {
			...
			/* Magog 10/18/2011 avoid pass by reference (&$configuration) -> ($configuration) */					
		...
		if( isset( $loginRes['login']['result'] ) ) {
			switch( $loginRes['login']['result'] ) {
				...
				case 'Throttled':
					...
					$recres = $this->__construct( $configuration, $this->extensions, $recursed + 1, null, $this->mwversion);
					return $recres;
					break;
				case 'Blocked':
				...
				case 'NeedToken':
					...
					$token = ["lgtoken"=> $loginRes["login"]["token"], "sessionid" => $loginRes["login"]["sessionid"]];
					
					$recres = $this->__construct( $configuration, $this->extensions, $recursed + 1, $token, $this->mwversion);
					return $recres;
					break;
				case 'Success':
	...
	public function apiQuery( $arrayParams = array(), $post = false, $errorcheck = true, $recursed = false) {
		...
		if (@$arrayParams['assert']) {
			Hooks::runHook( 'QueryAssert', array( &$arrayParams['assert'], &$assert ) );
		}
		
		if (@$logger && $logger->isTraceEnabled() && function_exists("map_array_function_keys")) {
			$asHttpString = implode('&', 
				array_map_pass_key($arrayParams, 
					function ($name, $value) {
						return rawurlencode($name) . '=' .
							 str_replace("%7C", "|", rawurlencode($value));
					}));
			$logger->trace("Running API query with params $this->base_url?$asHttpString\n\n");
		}
		...
		if( $post ) {
			...
			$rawResponse = $this->get_http()->post(
				$this->base_url,
				$arrayParams
			);
			
			$responseCode = $this->get_http()->get_HTTP_code();
			if ($responseCode < 200 || $responseCode >= 300) {
				$logger->error(
					"API Error...\n\nCode: $responseCode." . "\nText: $rawResponse");
				
				throw new APIError(
					array("code" => $responseCode, "text" => $rawResponse, 
						"info" => "Custom-generated APIError"));
			}
				
			$data = unserialize($rawResponse);

			/* Magog 12/6/2013 outputting response */
			if ($logger && $logger->isInsaneEnabled()) {
				$logger->insane("Response: ");
				$logger->insane($data);
			}
			...
			if( $assert && $data['edit']['assert'] == 'Failure' && $errorcheck ) {
				if ($logger) {
					...
					$logger->error( $data['edit']);
				}
				throw new APIError($data['error']);
			}
			
			$check_error_data = [$arrayParams, $data];
			$this->runHook( 'APIQueryCheckError', $check_error_data );
			...
			if( isset( $data['requestid'] ) ) {
				if( $data['requestid'] != $requestid ) {
					if( $recursed ) {
						if ($logger) {
							$logger->error("API Error... requestid's didn't match twice.");
						}
						return false;
					}
			...
			if( isset( $data['requestid'] ) ) {
				if( $data['requestid'] != $requestid ) {
					if( $recursed ) {
						if ($logger) {
							$logger->error( "API Error... requestid's didn't match "
									."twice.");
						}
						return false;
					}
	...
	public function get_tokens( $force = false ) {
		...
		if ($this->supports_version("1.24")) {
			$tokens = $this->apiQuery(["action" => "query", "meta" => "tokens"]);

			$token = _array_key_or_exception($tokens, ['query', 'tokens', 'csrftoken']);
			
			$this->tokens = array_fill_keys(explode("|", self::CSRF_TOKEN_TYPES), $token);
		} else {
			$tokens = $this->apiQuery(
				['action' => 'query', 'prop' => "info", 'titles' => 'Main Page', 
					'intoken' => self::CSRF_TOKEN_TYPES]);

			$tokens = array_merge_all(_array_key_or_exception($tokens, ['query', 'pages']));
			$this->tokens = map_array_function_keys($tokens, function ($val, $key) {
				if (in_string("token", $key)) {
					return [str_replace('token', '', $key), $val];
				}
			});
		}
	 ...
	 * @return void
	 */
	private function runHook($type, $data) {
	 ...
	 * @return boolean
	 */
	public function supports_version($version) {
		return version_compare($this->mwversion, $version) >= 0;
	}
	...
	public function __toString() {
		return $this->get_username().':'.$this->get_base_url();
	}
