...
class Cleanup_PD_User implements Cleanup_Module {
	...
	public function cleanup(Cleanup_Instance $ci) {
		...
		$ci->preg_replace(
			"/\{\{\s*PD\-user\-w\s*\|\s*(als|ar|bg|ca|cs|da|de|en|es|fi|fr|he|hi|hr|hu|it|ja|ka" .
		...
		$ci->preg_replace(
			"/\{\{\s*PD\-user\-w\s*\|\s*project\s*\=\s*wikipedia\s*\|\s*language\s*\=\s*" .
				 "(als|ar|bg|ca|cs|da|de|en|es|fi|fr|he|hi|hr|hu|it|ja|ka|lt|lv|ml|nl|nn|no|pl|pt|ro|" .
